...
Бэкенд для взаимодействия **заказчиков** и **исполнителей**: регистрация, вход по JWT, редактирование профиля, геолокация, справочник профессий, профиль исполнителя с ограничением по расстоянию, режим «на линии», создание заказов, диспетчеризация ближайшему онлайн‑работнику, чат по заказу, финансовые операции.
...
### Роли
- **employer** — пополняет баланс (`POST /transactions/deposit`), создаёт заказы (`POST /orders/`; **недоступно при отрицательном балансе**), смотрит историю (`GET /orders/my`), повторяет заказ (`POST /orders/{id}/repeat`), завершает (`PATCH .../complete`) или отменяет (`PATCH .../cancel`) заказ, пишет в чат, оставляет отзывы.
- **worker** — настраивает профиль (`PUT /workers/me/profile`), координаты (`PATCH /auth/me/location`), выходит на линию (`PATCH /workers/me/line`), получает предложения (`GET /orders/pending-offers`), отвечает на них (`POST /orders/offers/{offer_id}/respond`), пишет в чат, выводит заработок на карту (`POST /transactions/withdraw`).
...
- **`PATCH .../complete`** — завершение (доступно обоим участникам); автоматически списывает сумму с баланса заказчика и начисляет исполнителю за вычетом комиссии.
...
### Финансы
- **Заказчик**: пополняет баланс через `POST /transactions/deposit`. При отрицательном балансе создание новых заказов заблокировано (403). Баланс может уйти в минус при завершении заказа — это нормально, но новый заказ будет недоступен до погашения.
- **Исполнитель**: получает начисления после каждого завершённого заказа. Выводит деньги на карту через `POST /transactions/withdraw` (данные карты не сохраняются).
- **`GET /transactions/my`** — история операций с фильтром `?type=deposit|withdrawal|order_settlement`.
- **`GET /transactions/summary`** — сводка: текущий баланс, итого пополнено, выведено, заработано, потрачено.
...
OPENAPI_TAGS = [
    ...
    },
    {
        "name": "Транзакции",
        "description": (
            "Финансовые операции платформы. "
            "`POST /transactions/deposit` — пополнение баланса заказчика (только employer). "
            "`POST /transactions/withdraw` — вывод средств исполнителя на банковскую карту (только worker); "
            "данные карты валидируются, но не сохраняются. "
            "При завершении заказа автоматически создаётся расчёт (`order_settlement`): "
            "с заказчика списывается полная сумма, исполнителю начисляется за вычетом комиссии платформы. "
            "Баланс заказчика может уйти в минус — создание новых заказов при этом заблокировано. "
            "`GET /transactions/my?type=` — история с фильтром по типу (deposit / withdrawal / order_settlement). "
            "`GET /transactions/summary` — агрегированная сводка по всем операциям."
        ),
    },
    {
        "name": "Заказчики",
